...
## little per-particle wobble, never a direction of its own choosing.
...
## smooth drift. Long idle-ish stretches (just a slow sink), then a sudden
## impulse that decays fast.
func _step_daphnia(delta: float) -> void:
	for d in _daphnia:
		d.hop_timer -= delta
		if d.hop_timer <= 0.0:
			d.hop_timer = randf_range(0.3, 1.4)
			var direction := Vector2(randf_range(-0.4, 0.4), -1.0).normalized()
...
## sudden acceleration, a different rhythm than daphnia's constant jitter.
...
func _wrap(particle: Dictionary) -> void:
	particle.pos.x = wrapf(particle.pos.x, 0.0, size.x)
	particle.pos.y = wrapf(particle.pos.y, 0.0, size.y)
